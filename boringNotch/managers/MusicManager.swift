...
class MusicManager: ObservableObject {
    ...
    private var artworkFallbackTask: Task<Void, Never>? = nil
    ...
    private func updateFromPlaybackState(_ state: PlaybackState) {
        ...
        if hasContentChange {
            ...
            if artworkChanged, let artwork = state.artwork {
                ...
                // Don't swap to the app icon straight away. On a track skip the
                // new title and artist land before the artwork does, so reacting
                // immediately flashes the player's logo for a few hundred
                // milliseconds before the real cover arrives. Wait out a short
                // grace period and only fall back if artwork genuinely never
                // shows up (a stream or podcast with no cover).
                self.scheduleArtworkFallback(for: state.bundleIdentifier)
            }
    ...
    /// How long to keep showing the previous cover before giving up and using the
    /// player's app icon. Long enough to cover a normal skip, short enough that a
    /// genuinely artwork-less track doesn't sit on a stale cover.
    private static let artworkFallbackGrace: Duration = .milliseconds(900)

    /// Falls back to the player's app icon, but only if no artwork has arrived by
    /// the time the grace period elapses.
    private func scheduleArtworkFallback(for bundleIdentifier: String) {
        artworkFallbackTask?.cancel()

        // Already showing the icon for this player — nothing to schedule.
        if usingAppIconForArtwork && bundleIdentifier == lastArtworkBundleIdentifier { return }

        artworkFallbackTask = Task { @MainActor [weak self] in
            try? await Task.sleep(for: Self.artworkFallbackGrace)
            guard let self, !Task.isCancelled else { return }
            // Artwork may have arrived while we waited.
            guard self.artworkData == nil else { return }
            guard let appIconImage = AppIconAsNSImage(for: bundleIdentifier) else { return }
