...
extension Defaults.Keys {
    ...
    // MARK: Opened notch size
    // Upstream hard-coded these as `openNotchSize`. They are settings now, so the
    // expanded panel can be sized to taste. Bounds live in OpenNotchSizeLimits.
    ...
    /// Take a Dock icon while the settings window is open. Off keeps the app
    /// purely in the background, which is the point of a menu-bar utility.
