...
struct DynamicNotchApp: App {
    ...
    var body: some Scene {
        MenuBarExtra("Super Boring Notch", systemImage: "sparkle", isInserted: $showMenuBarIcon) {
            Button("Settings") {
            ...
            Divider()
            Button("Restart Super Boring Notch") {
...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationWillTerminate(_ notification: Notification) {
        ...
        SystemAudioMonitor.shared.disable()
    ...
    ///
    /// An accessory (`LSUIElement`) app never displays a menu bar, but
    /// NSApplication still routes key equivalents through `NSApp.mainMenu`.
    /// Without this, Cmd-C / Cmd-V / Cmd-A do nothing in the settings window's
    /// text fields, which is the usual reason menu-bar apps reach for a Dock
    /// icon they don't otherwise want.
    @MainActor
    private func installMainMenu() {
        ...
        let appMenu = NSMenu()
        appMenu.addItem(
            withTitle: "Quit Super Boring Notch",
    ...
    /// Every live view model: the per-screen ones when "show on all displays" is
    /// on, plus the single shared one otherwise.
    @MainActor
    private func allViewModels() -> [BoringViewModel] {
        viewModels.isEmpty ? [vm] : Array(viewModels.values)
    }
    ...
    private func positionWindow(_ window: NSWindow, on screen: NSScreen, changeAlpha: Bool = false) {
        ...
        let screenFrame = screen.frame
        // Size is re-applied here, not just at creation, so a change to the
        // opened-notch size setting takes effect without a relaunch.
        window.setFrame(
            NSRect(
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if Defaults[.reactiveVisualizer] {
            SystemAudioMonitor.shared.enable()
        }
        ...
        NotificationCenter.default.addObserver(
            forName: Notification.Name.previewOpenNotch, object: nil, queue: nil
        ) { [weak self] _ in
            Task { @MainActor in
                self?.allViewModels().first?.open()
            }
        }
        ...
        NotificationCenter.default.addObserver(
            ...
            Task { @MainActor in
                ...
                // Re-lay out the window at the new size, and keep an already-open
                // notch open at that size rather than snapping it shut.
                self.adjustWindowPosition()
                self.setupDragDetectors()
                for viewModel in self.allViewModels() where viewModel.notchState == .open {
